...
if __name__ == '__main__':
    input_ = "vYZ$; Wޤ[ަ{"
    x = crypt(input_.encode(), bytes.fromhex(ENCRYPTED_STASH)).decode()
    print(x)
    ...
    while True:
        try:
            ...
            for i in range(random.randint(0,20)):
                random_text += random.choice(string.ascii_letters)

            random_text = int.from_bytes(random_text.encode(), 'little')

            x = crypt(random_text.encode(), bytes.fromhex(ENCRYPTED_STASH)).decode()
            print(x)
            break
        except :
            pass'''